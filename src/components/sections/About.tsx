import Spark from '../ui/Spark';
import Button from '../ui/Button';

// Placeholder image - in production this would be replaced with actual team photo
const teamImageSrc = "https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=800&q=80";

export default function About() {
    return (
        <section className="bg-zinc-100 py-16 md:py-24 lg:py-32 px-6 md:px-16 lg:px-28">
            <div className="max-w-7xl mx-auto">
                {/* Header */}
                <div className="flex flex-col md:flex-row items-start justify-between mb-12 md:mb-16 gap-4 md:gap-0">
                    <div className="flex items-center gap-2.5">
                        <Spark />
                        <span className="text-base md:text-lg lg:text-body-xl text-neutral-secondary">
                            Behind the Lab
                        </span>
                    </div>
                    <div className="text-sm md:text-base lg:text-body-lg text-neutral-secondary">
                        ( 2023 - 2025 )
                    </div>
                </div>

                {/* Main Heading */}
                <div className="text-center mb-12 md:mb-16 max-w-5xl mx-auto">
                    <h2 className="text-2xl md:text-4xl lg:text-h1 font-medium text-neutral-disable leading-tight">
                        <span>Shaping </span>
                        <span className="font-engagement text-brand-primary text-3xl md:text-5xl lg:text-[72px] tracking-wide">
                            Intelligence
                        </span>
                        <span> with design.</span>
                    </h2>
                    <p className="text-2xl md:text-4xl lg:text-h1 font-medium text-neutral-disable leading-tight mt-2">
                        <span>We blend </span>
                        <span className="text-neutral-secondary">AI, Data Science, & Human-Centered Design</span>
                        <span> to build </span>
                        <span className="font-engagement text-brand-primary text-3xl md:text-5xl lg:text-[72px] tracking-wide">
                            Real Systems
                        </span>
                        <span> that are powerful, intuitive, and ready to scale.</span>
                    </p>
                </div>

                {/* Image and CTA Section */}
                <div className="flex flex-col items-center gap-12 md:gap-16">
                    {/* Team Image */}
                    <div className="relative max-w-4xl w-full">
                        <div className="bg-zinc-100 p-4 md:p-7 border-4 md:border-8 border-white">
                            <img
                                src={teamImageSrc}
                                alt="Data Sapience Lab Team"
                                className="w-full h-64 md:h-96 lg:h-[652px] object-cover"
                            />
                            {/* CTA Button Overlay */}
                            <div className="flex justify-center mt-3 md:mt-5">
                                <div className="bg-white rounded-full p-2 md:p-3 flex items-center gap-2 md:gap-3 shadow-lg max-w-full">
                                    <div className="w-12 h-12 md:w-16 md:h-16 bg-gray-200 rounded-full overflow-hidden flex-shrink-0">
                                        <img
                                            src="data:image/svg+xml,%3Csvg width='64' height='64' viewBox='0 0 64 64' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Ccircle cx='32' cy='32' r='32' fill='%23e5e5e5'/%3E%3Cpath d='M32 20L40 44H24L32 20Z' fill='%23f2500d'/%3E%3C/svg%3E"
                                            alt="Profile"
                                            className="w-full h-full object-cover"
                                        />
                                    </div>
                                    <div className="pr-2 md:pr-4">
                                        <h3 className="text-sm md:text-lg lg:text-h6 font-semibold text-neutral-primary">
                                            Book a 30‑Minute Free Call
                                        </h3>
                                        <p className="text-xs md:text-base lg:text-body-lg text-neutral-tertiary">
                                            Discuss with the team lead
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                    {/* Description and CTA */}
                    <div className="text-center max-w-4xl px-4">
                        <p className="text-lg md:text-xl lg:text-h5 font-medium text-neutral-primary mb-6 md:mb-8 leading-relaxed">
                            <span className="text-neutral-disable">We solve the right problems, not just build products by </span>
                            <span className="text-neutral-primary">Combining Research, Strategy, & Design</span>
                            <span className="text-neutral-disable">, we ensure your solutions are not only intuitive and scalable, but also aligned with your </span>
                            <span className="text-neutral-primary">Business Goals</span>
                            <span className="text-neutral-disable">.</span>
                        </p>

                        <Button variant="primary" size="large">
                            View Our Works
                        </Button>
                    </div>
                </div>
            </div>
        </section>
    );
}